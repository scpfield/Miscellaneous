...
# class with wrapped versions that use a shared mutex _Lock_ to protect
...
# instance of a class has its own distinct _Lock_ that applies to all the 
...
# threads from executing the function at the same time by using a _Lock_
...
def Synchronized( Target ):
    ...
    def Wrapper( *args, **kwargs ):
        ...
        if type( Target ) == type:
            ...
            # versions that will synchronize on a shared _Lock_.
            #
            ...
            if not hasattr( Target, 'ClassSynchronized' ):
                ...
                for MemberName, MemberFunction in inspect.getmembers_static( Target ):
                    ...
                    # Also, skip __init__ and __new__, which don't need any _Lock_ protection.                    
                    ...
                    if (( '<method'   in str( MemberFunction).strip() ) or
                        ...
                        ( '<slot'     in str( MemberFunction).strip() )):
                            ...
                            SynchronizedFunction.MemberSynchronized = True
            ...
            # Finally, create an R_Lock_ and add it to the new object instance
            # as one of its attributes.  Need to use object.__setattr_()
            # instead of setattr() or else infinite recursion.
            #
            # All the member functions we replaced will share this same _Lock_,
            ...
            if hasattr( Wrapper, 'MemberSynchronized' ):
                ...
                #
                # First we need to find the _Lock_ object.
                #
                ...
                #_Lock_ = args[0]._Lock_
                
            else:
            
                # For standalone functions, the first time the function is called 
                # we create a _Lock_ object for it and store it in an attribute of
                # the function.  Subsequent calls to ths same function will have 
                # the _Lock_ ready to go.  
            ...
            with _Lock_:
                ...
                builtins.print( f'{CallerInfo}  Acquired Lock ID:{id(_Lock_)}, Waiting For Event ID: {id(_Event_)} For Function: {Target.__name__}' )
                                
                _Event_.wait()
                
                builtins.print( f'{CallerInfo}  Event Is Signaled For Event ID: {id(_Event_)} For Function: {Target.__name__}' )
                                
                _Event_.clear()
                
                builtins.print( f'{CallerInfo}  Event Is Cleared For Event ID: {id(_Event_)}, Calling Function: {Target.__name__}' )
            ...
            builtins.print( f'{CallerInfo}  Released Lock ID:{id(_Lock_)}, Signaling Event ID: {id(_Event_)}' )
